...
from PyQt5.QtCore import QSettings, QByteArray, QObject, QEvent
from PyQt5.QtGui import QKeySequence
from PyQt5.QtWidgets import QWidget, QMenuBar, QProgressBar, QApplication
...
from pymodaq_plugins_readspectro.utils import Config as PluginConfig
...
EXTENSION_NAME = 'Read Spectrometer'
CLASS_NAME = 'ReadSpectro'


from PyQt5.QtWidgets import QMainWindow

class SpectrometerMain(QMainWindow):
...
class ReadSpectro(CustomExt):
    ...
    def __init__(self, parent: DockArea, dashboard):
        ...
        # info: in an extension, if you want to interact with ControlModules
        # you have to use the object: self.modules_manager which is a
        # ModulesManager instance from the dashboard

        self.setup_ui()
        self.close_event_filter = CloseEventFilter(self)
        self.mainwindow.installEventFilter(self.close_event_filter)
        ...
        self.dashboard.parent.parent().close()
    ...
    def setup_docks(self):
        ...
        self.detector = DAQ_Viewer(self.daq_viewer_area, title="Spectrometer")
        self.detector.daq_type = 'DAQ1D'
        self.detector.detector = \
            self.dashboard.detector_modules[0].detector
        #.get_mod_from_name("Avantes", "det")
    ...
    def take_background(self):
        ...
        self.adjust_actions()
        dfp = DataFromPlugins(name='Spectro', data=self.background, dim='Data1D',
                              labels=['background'])
        self.spectrum_viewer.show_data(dfp)
    ...
    def take_reference(self):
        ...
        self.adjust_actions()
        dfp = DataFromPlugins(name='Spectro', data=self.reference, dim='Data1D',
                              labels=['data'])
        self.spectrum_viewer.show_data(dfp)
        dfp = DataFromPlugins(name='Spectro', data=self.reference, dim='Data1D',
                              labels=['reference'])
    ...
    def read_settings(self):
        ...
        settings = QSettings("pymodaq", "read-spectrometer")
...
def main():
    ...
    try:
        preset_file_name = \
                  plugin_config('presets', f'preset_for_{CLASS_NAME.lower()}')
